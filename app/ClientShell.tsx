...
import { 
  ...
  Settings, LogOut, ChevronLeft, ChevronRight, ChevronDown, User,
  Gavel, ShoppingBag, FileSignature, Megaphone
} from "lucide-react";
...
export default function ClientShell({ children }: { children: React.ReactNode }) {
  ...
  const isPublicRoute = pathname === '/login' || pathname === '/portal-escola' || pathname === '/participante';

  if (isPublicRoute) {
    return <div className="min-h-screen bg-axon-bg text-white">{children}</div>;
  }
    ...
    <div className="flex h-screen overflow-hidden bg-axon-bg text-white">
      <aside className={`${isSidebarOpen ? "w-64" : "w-20"} bg-axon-panel border-r border-axon-border flex flex-col transition-all duration-300 ease-in-out relative z-20`}>
        ...
        <nav className="flex-1 py-6 flex flex-col gap-2 px-3 overflow-y-auto">
          {navLinks.map((link) => {
            ...
            return (
              <Link key={link.href} href={link.href} className={`flex items-center gap-3 rounded-md transition-colors ${isActive ? 'bg-axon-green-dim text-axon-green font-medium' : 'text-gray-400 hover:text-white hover:bg-white/5'} ${isSidebarOpen ? 'px-3 py-2' : 'p-3 justify-center'}`}>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        <header className="h-16 bg-axon-panel/80 backdrop-blur-md border-b border-axon-border flex items-center justify-between px-8 sticky top-0 z-10">
          <div className="text-gray-400 text-sm font-medium">Visão do Organizador</div>
          <div className="relative">
            <button onClick={() => setIsUserMenuOpen(!isUserMenuOpen)} className="flex items-center gap-3 hover:bg-white/5 p-1.5 pr-3 rounded-full transition-colors border border-transparent hover:border-axon-border">
              ...
              <div className="text-left hidden sm:block">
                <div className="text-sm font-medium text-white leading-tight">Usuário AXON</div>
            ...
            {isUserMenuOpen && (
              <>
                ...
                <div className="absolute right-0 mt-2 w-56 bg-axon-panel border border-axon-border rounded-xl shadow-2xl z-50 py-2 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="px-4 py-2 border-b border-axon-border mb-2">
                    <p className="text-sm text-white font-medium">Minha Conta</p>
                    <p className="text-xs text-gray-400 truncate">admin@axongroup.com</p>
                  </div>
                  <a href="#" className="flex items-center gap-3 px-4 py-2 text-sm text-gray-300 hover:text-white hover:bg-white/5 transition-colors"><User size={16} /> Meu Perfil</a>
                  <a href="#" className="flex items-center gap-3 px-4 py-2 text-sm text-gray-300 hover:text-white hover:bg-white/5 transition-colors"><Settings size={16} /> Configurações</a>
                  <div className="h-px bg-axon-border my-2" />
                  <a href="#" className="flex items-center gap-3 px-4 py-2 text-sm text-red-400 hover:text-red-300 hover:bg-red-400/10 transition-colors"><LogOut size={16} /> Sair da plataforma</a>
                </div>
              </>
            )}
          </div>
