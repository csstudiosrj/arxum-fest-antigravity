...
function ToastContainer({ toasts, remover }: { toasts: Toast[]; remover: (id: number) => void }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[100] flex flex-col gap-2 pointer-events-none">
      {toasts.map((t) => (
        <div key={t.id} className={`flex items-center gap-3 px-4 py-3 rounded-xl border shadow-2xl pointer-events-auto transition-all ${cores[t.tipo]}`}>
          {icones[t.tipo]}
...
export default function PainelEventoPage() {
  ...
  useEffect(() => {
    async function carregar() {
      ...
      const [{ data: ev }, { data: cats }, { data: apres }] = await Promise.all([
        ...
        supabase
          .from("apresentacoes")
          .select("id, nome, tipo, ordem_apresentacao")
  ...
  async function salvarCategoria() {
    ...
    if (catEditando) {
      ...
      if (!error) { addToast("sucesso", `"${formCat.nome}" atualizada!`); recarregarCategorias(); }
      else addToast("erro", "Erro ao atualizar categoria.");
    } else {
      const { error } = await supabase.from("categorias").insert(payload);
      if (!error) { addToast("sucesso", `"${formCat.nome}" criada!`); recarregarCategorias(); }
      else addToast("erro", "Erro ao criar categoria.");
    }
    setModalCat(false);
  }
  ...
  async function excluirCategoria(id: string, nome: string) {
    ...
    if (!error) { addToast("sucesso", `"${nome}" removida.`); recarregarCategorias(); }
    else addToast("erro", "Erro ao remover categoria.");
  }
          ...
          <div className="p-8">
              ...
              <div className="space-y-8">
                ...
                <div>
                  ...
                  <div className="bg-axon-bg border border-axon-border rounded-xl divide-y divide-axon-border">
                    ...
                    ].map(({ label, descricao, ok, acao, acaoLabel }) => (
            ...
            {abaAtiva === "configuracoes" && (
              <div className="space-y-6">
                ...
                <div className={`rounded-xl p-4 border text-sm flex items-start gap-2.5 ${
                  form.status === "inscricoes_abertas" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-300" :
                  form.status === "em_andamento"       ? "bg-axon-gold/10 border-axon-gold/20 text-axon-gold" :
                  form.status === "encerrado"          ? "bg-red-400/10 border-red-400/20 text-red-300" :
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {cat.faixa_etaria_label && (
                                <span className="text-xs text-gray-500 bg-white/5 border border-white/10 px-2 py-0.5 rounded-full">
                                  {cat.faixa_etaria_label}
                                  {cat.faixa_etaria_min && cat.faixa_etaria_max ? ` (${cat.faixa_etaria_min}–${cat.faixa_etaria_max} anos)` : ""}
                                </span>
                              )}
                            </div>
                                  ...
                                  <div>
                                    <div className="flex items-center gap-2 flex-wrap">
                                      ...
                                      {sub.faixa_etaria_label && (
                                        <span className="text-xs text-gray-500 bg-white/5 border border-white/10 px-2 py-0.5 rounded-full">
                                          {sub.faixa_etaria_label}
                                          {sub.faixa_etaria_min && sub.faixa_etaria_max ? ` (${sub.faixa_etaria_min}–${sub.faixa_etaria_max} anos)` : ""}
                                        </span>
                                      )}
                                    </div>
            ...
            <div className="bg-axon-panel border border-axon-border rounded-xl w-full max-w-lg p-6 space-y-5 max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-1 gap-4">
                {!formCat.categoria_pai_id && !catEditando?.categoria_pai_id && (
                  <div className="space-y-2">
                    <label className="text-sm text-gray-400 font-medium">Categoria Pai <span className="text-gray-600 font-normal">(opcional)</span></label>
                ...
                <div className="space-y-2">
                  <label className="text-sm text-gray-400 font-medium">Faixa Etária <span className="text-gray-600 font-normal">(opcional)</span></label>
                  <div className="grid grid-cols-3 gap-3">
                    <div className="space-y-1">
                      <label className="text-xs text-gray-500">Label</label>
                      <input type="text" placeholder="Ex: Infantil"
