...
// ─── Gerador de QR Code PIX (padrão EMV, sem API externa) ────────────────────
  ...
  return (
    <div className="fixed inset-0 bg-axon-bg flex flex-col items-center justify-center gap-8 p-6">
      <div className="flex flex-col items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-white">Acesso ao Caixa</h1>
            ...
            className={`w-4 h-4 rounded-full border-2 transition-all ${
              pin.length > i
                ? erro
                  ...
                  : "bg-axon-green border-axon-green"
                : "border-axon-border"
            }`}
          />
        ))}
...
function ModalPIX({
  ...
  useEffect(() => {
    ...
    // Gera QR via API pública do Google Charts (sem backend)
    setQrUrl(
      `https://chart.googleapis.com/chart?chs=300x300&cht=qr&chl=${encodeURIComponent(emv)}&choe=UTF-8`
    );
  }, [total, config]);
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-axon-panel border border-axon-border rounded-2xl p-6 w-full max-w-sm space-y-5">
        ...
        {semConfig ? (
          <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4 text-center">
            <p className="text-yellow-400 text-sm">
              Configure a chave PIX em <strong>PDV → Configurações</strong> para gerar o QR Code.
            </p>
          </div>
        ) : (
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <button
            onClick={onConfirmar}
            className="py-3 rounded-xl bg-axon-green text-black font-bold text-sm hover:bg-[#00c866] transition-colors flex items-center justify-center gap-2"
          >
      ...
      <div className="bg-axon-panel border border-axon-border rounded-2xl p-6 w-full max-w-sm space-y-5">
        ...
        {valorNum >= total && (
          <div className="bg-axon-green/10 border border-axon-green/30 rounded-xl p-4 text-center">
            ...
            <p className="text-3xl font-bold text-axon-green">
...
export default function CaixaPage() {
  ...
  if (!autenticado) {
    return (
      <TelaPIN
        pinCorreto={config.pin_vendedor}
      ...
      <div className="h-screen flex flex-col bg-axon-bg overflow-hidden">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            {/* Grid de produtos — botões grandes */}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                {produtosFiltrados.map((p) => (
                  <button
                    key={p.id}
                    onClick={() => adicionar(p)}
                    className="bg-axon-panel border border-axon-border rounded-2xl p-4 flex flex-col items-center justify-center text-center h-28 sm:h-32 active:scale-95 hover:border-axon-green transition-all group"
                  >
          ...
          <div className="w-72 lg:w-80 bg-axon-panel border-l border-axon-border flex flex-col shrink-0">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-axon-border">
              <div className="flex items-center gap-2 text-white font-medium text-sm">
                <ShoppingCart size={16} className="text-axon-green" />
                Pedido
                {carrinho.length > 0 && (
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
              {carrinho.length === 0 ? (
                ...
                carrinho.map((item) => (
                  <div
                    key={item.produto.id}
                    className="flex items-center justify-between bg-axon-bg border border-axon-border rounded-xl px-3 py-2.5"
                  >
